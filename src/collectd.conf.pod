...
=head2 Plugin C<diskpath>

The C<diskpath> plugin collects information about the usage of disk paths.
Values collected are the number of octets written to and read from a disk path,
the number of read/write operations issued to the disk path and a rather complex
"time" it took for these commands to be issued.

Using the following two options you can ignore some disk paths or configure the
collection only of specific disk paths.

=over 4

=item B<DiskPath> I<Name>

Select the disk path I<Name>. Whether it is collected or ignored depends on the
B<IgnoreSelected> setting, see below. As with other plugins that use the
daemon's ignorelist functionality, a string that starts and ends with a slash
is interpreted as a regular expression. Examples:

  DiskPath "hdisk0_Path0
  DiskPath "/hdisk1_Path[0-9]/"

=item B<IgnoreSelected> B<true>|B<false>

Sets whether selected disk paths, i.E<nbsp>e. the ones matches by any of the
B<DiskPath> statements, are ignored or if all other disk paths are ignored.
The behavior (hopefully) is intuitive: If no B<DiskPath> option is configured,
all disk paths are collected. If at least one B<DiskPath> option is given and
no B<IgnoreSelected> or set to B<false>, B<only> matching disk paths will be
collected. If B<IgnoreSelected> is set to B<true>, all disk paths are collected
...
The C<hba> plugin use the library I<libHBAAPI> to collect  information about
the usage of host bus adapters. Values collected are the number of octets
written to and read from a hba, the number of read/write operations issued to
the hba and mumber of received/transmitted frames from a hba.

In I<AIX> the library I<libHBAAPI> is not used for efficiency.

The plugin only collect data from Fibre Channel adapters.

Using the following two options you can ignore some hbas or configure the
collection only of specific hbas.

=over 4

=item B<Hba> I<Name>

Select the hba I<Name>. Whether it is collected or ignored depends on the
B<IgnoreSelected> setting, see below. As with other plugins that use the
daemon's ignorelist functionality, a string that starts and ends with a slash
is interpreted as a regular expression. Examples:
...
Sets whether selected hba, i.E<nbsp>e. the ones matches by any of the B<Hba>
statements, are ignored or if all other hbas are ignored. The behavior
(hopefully) is intuitive: If no B<Hba> option is configured, all hbas are
collected. If at least one B<Hba> option is given and no B<IgnoreSelected> or
set to B<false>, B<only> matching hbas will be collected. If B<IgnoreSelected>
is set to B<true>, all hbas are collected B<except> the ones matched.
...
The I<mongodb plugin> will collect stats from I<MongoDB>, a schema-less
NoSQL database.
...
The plugin read stats from I<MongoDB> database.
...
=item B<PreferSecondaryQuery> B<false>|B<true>
...
than those of other plugins. It usually looks something like this:
  ...
  <Plugin odbc>
    ...
    <Database "product_information">
      Interval 120
      Connection "Server=localhost;Port=1234;Database=prod_info;Uid=collectd;Pwd=aZo6daiw;"
      PingQuery "SELECT 1"
      Query "out_of_stock"
    </Database>
  </Plugin>
...
The database version is determined assuming that each part of the version is
in the range from B<00> to B<99> and all dots are removed. So version "4.1.2"
becomes "40102", version "5.0.42" becomes "50042".
